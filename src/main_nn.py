...
nn = RESUS_NN(num_fields, COLD_USER_THRESHOLD, encoder, model).to(device)
optimizer = torch.optim.Adam(
    [
        {"params": nn.encoder.parameters(), "lr": 0.001},
        {"params": nn.fc1.parameters(), "lr": 0.001},
        {"params": nn.adjust.parameters(), "lr": 0.01},
    ],
)
...
for epoch in range(num_epochs):
    ...
    for itr, batch in enumerate(tqdm(train_query_dataloader)):
        ...
        outputs, _ = nn(support_data, query_data)
        #         outputs, predictor, delta_W_X, delta_W = nn(feature_ids, feature_vals, support_data)
    ...
    nn.eval()
    val_loss, val_auc, val_gauc = val_query(nn, val_query_dataloader, gauc_col=0, device=device)
    ...
    if val_auc > best_auc:
        ...
        best_epoch = epoch
        torch.save(state, f"checkpoint/RESUS_NN-fast-{dataset}-{exp_id}.tar")
    else:
        patience -= 1
...
for i in range(1,COLD_USER_THRESHOLD+1,1):
    ...
    test_loss, test_auc, test_gauc = val_query(nn, test_query_dataloader, gauc_col=0, device=device)
    print(f"test loss of user group {i}: {test_loss}, auc: {test_auc}, gauc: {test_gauc}")

    nn_test_losses += [test_loss]
    nn_test_aucs += [test_auc]

print('test losses')
for loss in nn_test_losses:
    print(loss)
print('test aucs')
for loss in nn_test_aucs:
    print(loss)

print(f"resus_nn cold start I: Loss: {sum(nn_test_losses[:10])/10}, auc: {sum(nn_test_aucs[:10])/10}")
print(f"resus_nn cold start II: Loss: {sum(nn_test_losses[10:20])/10}, auc: {sum(nn_test_aucs[10:20])/10}")
print(f"resus_nn cold start III: Loss: {sum(nn_test_losses[20:30])/10}, auc: {sum(nn_test_aucs[20:30])/10}")
